#pragma once
	...
	#define TAVERN_ENGINE_ASSERT(x, ...) { if(!(x)) { TAVERN_ENGINE_ERROR("Assertion Failed: {0}", __VA_ARGS__); __debugbreak(); } }
	#define TAVERN_ASSERT(x, ...) { if(!(x)) { TAVERN_ERROR("Assertion Failed: {0}", __VA_ARGS__); __debugbreak(); } }
#else
